...
function canvasMap(id,regional=false){
 ...
 else{ctx.fillStyle='#466575';ctx.font='12px system-ui';for(const[label,lon,lat]of[['ARGENTINA',-65,-41],['URUGUAY',-56,-33],['ATLÁNTICO SUR',-49,-46]]){const p=projection([lon,lat]);ctx.fillText(label,p[0],p[1]);}}
...
function timeline(area,id){
 ...
 const plot=Plot.plot({width,height:335,marginLeft:55,marginTop:58,marginRight:25,title:area==='global'?'Global · escala propia':'Atlántico sudoccidental',style:{fontFamily:'DM Sans, system-ui',fontSize:'12px'},
 ...
 ...rules.map(r=>Plot.text([r],{x:'date',text:'label',frameAnchor:'top',textAnchor:width<600?'middle':'start',dx:width<600?0:6,dy:-26,fontSize:width<600?9:11,fill:r.color,stroke:'white',strokeWidth:3}))]});
