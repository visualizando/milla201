...
function canvasMap(id,regional=false){
 ...
 ctx.fillStyle=palette.paper;ctx.beginPath();path({type:'Sphere'});ctx.fill();
 ctx.strokeStyle=palette.border;ctx.lineWidth=.5;ctx.beginPath();path(d3.geoGraticule10());ctx.stroke();
 ...
 const color=d3.scaleLog().domain([1,100]).range([palette.mint,palette.ink]).clamp(true);
 ...
 ctx.fillStyle=palette.surface;ctx.strokeStyle=palette.border;ctx.lineWidth=.6;ctx.beginPath();path(state.land);ctx.fill();ctx.stroke();
 if(!regional){ctx.strokeStyle=palette.ink;ctx.lineWidth=1;ctx.setLineDash([4,3]);ctx.beginPath();path(bbox);ctx.stroke();ctx.setLineDash([]);}
 else{ctx.fillStyle=palette.ink;ctx.font='12px Montserrat, system-ui';for(const[label,lon,lat]of[['ARGENTINA',-65,-41],['URUGUAY',-56,-33],['ATLÁNTICO SUR',-49,-46]]){const p=projection([lon,lat]);ctx.fillText(label,p[0],p[1]);}}
...
function timeline(area,id){
 ...
 const rules=[{date:new Date('2020-01-01T00:00:00Z'),label:'Fin del período original\n2017–2019',color:palette.ink},{date:new Date('2023-12-10T00:00:00Z'),label:'Asunción de Milei\n10 dic. 2023',color:palette.ink}];
 ...
 y:{label:labels[metric],zero:true,grid:true},marks:[Plot.ruleY([0],{stroke:palette.border}),
 ...rules.map(r=>Plot.ruleX([r.date],{stroke:r.color,strokeDasharray:'4,4'})),
 Plot.lineY(rows,{x:'date',y:metric,stroke:palette.ink,strokeWidth:1.8}),
 Plot.dot(rows,{x:'date',y:metric,fill:palette.ink,r:2.5,title:d=>`${d.month}: ${fmt.format(d[metric])}`}),
